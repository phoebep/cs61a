...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current = self.place 
        dist = 0
        while not current.entrance==hive:
            current = current.entrance
            dist += 1 
            if current.bees and dist>=self.min_range and dist<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor<=0:
            self.place.bees = [b for b in [hurt(bee) for bee in self.place.bees] if b.armor>0]
            self.place.remove_insect(self)
        # END Problem 5
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while not curr_place==None:
            if not curr_place.ant == None:
                ant = curr_place.ant 
                if ant.is_container:
                    baby = ant.contained_ant 
                else:
                    baby = None 
                if not ant.damage_doubled:
                    ant = queen_hit(ant)
                if (not baby==None) and (not baby.damage_doubled):
                    baby = queen_hit(baby)
