...
def balanced(m):
    ...
    if is_mobile(end(left(m))):
        left_bal = balanced(end(left(m)))
    if is_mobile(end(right(m))):
        right_bal = balanced(end(right(m)))
    return length(left(m))*total_weight(end(left(m))) == length(right(m))*total_weight(end(right(m))) and left_bal and right_bal
...
def generate_paths(t, x):
    ...
    if label(t) == x:
        yield [x] 
    for b in branches(t):
        b_list = list(generate_paths(b, x))
        yield from [[label(t)] + b_list[i] for i in range(len(b_list))]
